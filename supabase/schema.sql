...
create policy "Users view own collection" on public.collection for select using (auth.uid() = user_id);
...
create policy "Friends can view collection" on public.collection for select
  using (
    auth.uid() = user_id or
    exists (
